...
contract Factory is BaseSplitCodeFactory, IFactory {
  ...
  function createPool(
    ...
    pool = _create(
      bytes(""),
      keccak256(abi.encode(token0, token1, swapFeeUnits))
    );
  ...
  function addNFTManager(
    address _nftManager
  ) external onlyConfigMaster returns (bool added) {
  ...
  function removeNFTManager(
    address _nftManager
  ) external onlyConfigMaster returns (bool removed) {
  ...
  function enableSwapFee(
    ...
    require(
      tickDistance > 0 && tickDistance < 16384,
